...
class LayerNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        _mean = (x.sum((1, )) / x.shape[1]).reshape((x.shape[0], 1)).broadcast_to(x.shape)
        _var = (((x - _mean) ** 2).sum((1, )) / x.shape[1]).reshape((x.shape[0], 1)).broadcast_to(x.shape)
        _deno = (_var + self.eps) ** 0.5
        return self.weight.broadcast_to(x.shape) * (x - _mean)/_deno + self.bias.broadcast_to(x.shape)
...
class Residual(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        assert self.fn(x).shape == x.shape
        return self.fn(x) + x