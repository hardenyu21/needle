...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        Args:
            new_shape (tuple): new shape of the array, allow only one element equals to -1
        ...
        if -1 in new_shape:
            counter = Counter(new_shape)
            if counter[-1] > 1:
                raise ValueError("Only one dimension can be inferred from the given shape.")
            if old_size % new_size:
                raise ValueError(f"Cannot reshape array of size {self.shape} into shape {new_shape}")
            new_shape = tuple([dim if dim != -1 else int(-old_size / new_size) for dim in new_shape])
        else:
            if old_size != new_size:
                raise ValueError(f"Cannot reshape array of size {self.shape} into shape {new_shape}")
    ...
    def permute(self, new_axes):
        ...
        if len(old_shape) != len(new_axes):
            raise ValueError("The number of dimensions should be the same after permutation")
    ...
    def broadcast_to(self, new_shape):
        ...
        assert len(new_shape) == len(self.shape), "The number of dimensions must be the same"
        for dim1, dim2 in zip(new_shape, self.shape):
            assert (dim1 == dim2 or dim2 == 1), f"cannot broadcast shape {self.shape} to {new_shape}"
