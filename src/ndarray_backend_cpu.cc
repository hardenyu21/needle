...
void EwiseMul(const AlignedArray& a, const AlignedArray& b, AlignedArray* out){
      ...
      /**
   * Element-wise Multiply two (compact) matrices into an output (also compact) matrix.  
   *
   * Args:
   *   a: compact 2D array of size m * n
   *   b: compact 2D array of size m * n
   *   out: compact 2D array of size m * n to write the output to
   */
    assert(a.size == b.size);
    for (size_t i = 0; i < a.size; i++){
        out->ptr[i] = a.ptr[i] * b.ptr[i];
...
{
    /**
   * Scalar Multiply (compact) matrix into an output (also compact) matrix.  
   *
   * Args:
   *   a: compact 2D array of size m * n
   *   val: multiplication scalar
   *   out: compact 2D array of size m * n to write the output to
   */
    for (size_t i =0; i < a.size; i++) {
        out->ptr[i] = a.ptr[i] * val;
